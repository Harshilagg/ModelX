...
class _ScoutPageState extends State<ScoutPage> {
  ...
  Future<void> _search() async {
    ...
    if (_locationCtl.text.trim().isNotEmpty)
      filters['location'] = _locationCtl.text.trim();
  ...
  Future<void> _aiSearch() async {
    ...
    if (query.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Describe who you\'re looking for first.'),
        ),
      );
      return;
    }
    ...
    try {
      ...
      if (results.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'No AI matches found for that search — try being less specific.',
            ),
          ),
        );
      }
                ...
                Expanded(
                  child: TextField(
                    ...
                    decoration: const InputDecoration(
                      ...
                      hintText: 'Try "tall models in Delhi for editorial"...',
                    ),
                          ...
                          child: ListView.separated(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              return Row(
                                children: [
                                  Expanded(
                                    child: ModelCard(data: data, compact: true),
                                  ),
                                  const SizedBox(width: 8),
                                  IconButton(
                                    onPressed: () =>
                                        _toggleShortlist(data['id']),
