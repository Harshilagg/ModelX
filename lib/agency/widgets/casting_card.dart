...
class _CastingCardState extends State<CastingCard> {
          ...
          child: Column(
            ...
            children: [
              ...
              Wrap(
                ...
                children: [
                  if (widget.timeline.isNotEmpty)
                    _detailText('Timeline: ${widget.timeline}'),
                  if ((widget.compensationMin != null &&
                          widget.compensationMin!.isNotEmpty) ||
                      (widget.compensationMax != null &&
                          widget.compensationMax!.isNotEmpty))
                    _detailText(
                      'Compensation: ${widget.compensationMin ?? '-'} - ${widget.compensationMax ?? '-'}',
                    ),
                  if (widget.budgetType.isNotEmpty &&
                      widget.budgetAmount.isNotEmpty)
                    _detailText(
                      '${widget.budgetType}: ₹${widget.budgetAmount}',
                    ),
                  if (widget.location.isNotEmpty)
                    _detailText('Location: ${widget.location}'),
                ],
              ),
              ...
              Row(
                children: [
                  Text(
                    '${widget.applicants} applications',
                    style: const TextStyle(
                      fontWeight: FontWeight.w700,
                      fontSize: 13,
                      color: AppColors.ink,
  ...
  Widget _talentSummary(Map<String, dynamic> t) {
    ...
    final looks = (looksRaw is List)
        ...
        : looksRaw?.toString();
    final skills = (t['skills'] is List)
        ? (t['skills'] as List).cast<String>()
        : null;
    final parts = <String>[];
